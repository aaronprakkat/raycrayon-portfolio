...
- Files:
  ...
  - `js/gumi-title.js` the Gumizoo title's live gummy WebGL shader
...
3. **Selected work:** personal projects.
   ...
   - Gumizoo: a centered composition on the site's plain cream background (no section background of its own) — "Gumizoo" heading (thick, glossy gummy-candy lettering, pearl gray at rest; see "The gummy title" below. It keeps the old Bebas line box height, so it never outweighs the heads) and the tagline "Four forms of stupidity" beneath it, both centered; below that the four heads (Ashik Rancher, Gia Tangerine, Nanju Jam, Rana Ghee) at their real render aspect (`--gumi-ar`, 1230:963 — uncropped, no letterboxing), 2×2 on phones and one row of four on desktop (900px+), also centered; a shared materials note ("PLA+ filament, wood filler, primer, acrylic paint.") below that, centered too. The event poster asset (`media/stills/gumizoo.webp`) is no longer part of this section's visible layout — `main.js` still unconditionally fetches and appends it to `.gumizoo__poster` on init, so that element stays in the DOM, just `display: none`. Desktop: sized to fit one viewport with no scroll, the same `min()`-of-a-width-and-a-height-budget approach as before, just without a poster term. Four-across is more width-hungry than the old 2×2 was, so — with no poster competing for width — this row ends up width-bound rather than height-bound, hitting the standard `--max` (1400px) cap on wide/short screens with a lot of spare height left over (which the section's own vertical centering turns into calm whitespace, not a cramped row): tiles come out to 338×265px at 1920×1080 (row at 72.9% of viewport, the full 1400px cap) and 292×229px at 1366×768 (row at 89.0% of viewport, gutter-limited there, short of the cap). Gaps: 16px on desktop (12px on phones). Frames: 2px ink border (`rgba(17,17,17,.78)`) and a soft drop shadow. Tile texture: at rest a light neutral grainy gradient (off-white, grain at 42% opacity, `soft-light` blend — not `multiply`, which only ever darkens and read too heavy/saturated at rest); on hover/focus that tile's accent gradient fades in under the same grain (opacity up to .5), so it's one texture going from quiet to bold. The section background never floods; only the hovered tile does. Phones: heading, tagline, grid, materials note stacked, still centered.
     - **The gummy title is live WebGL** (`js/gumi-title.js`, raw WebGL, no library). The `<h3>` keeps its real text "Gumizoo" (`.gumi-title__text`, Bagel Fat One at .8em, loaded from Google Fonts subset to `text=GUMIZO`, a few KB; the only use of that face). At runtime the word is drawn onto an offscreen canvas in the same font, size and letter-spacing, and turned into a signed distance field (an exact Euclidean distance transform). That gives a pillow height field (quarter-circle profile, radius about 92% of the thickest stroke's half-width, lightly blurred) whose normals, signed distance and thickness are packed into one RGBA texture. One fragment shader on an `aria-hidden` canvas over the text shades it: a solid side wall stepped back along a fixed down-right direction (40 steps, depth .11em), darker toward its far edge; gelatin translucency, with thin rims lighter and more luminous and the thick core deeper (`pow(base, .5 + .72 * thickness)`), plus light coming through the lower edges; a crisp and a broad highlight from a fixed upper-left light, a little Fresnel, and a thin darker silhouette line. There's no bloom. The text stays in place (transparent), so the layout is identical with or without the canvas.
       - **Colour:** pearl gray at rest (`--title-rest: #C9C6BF` on `.gumizoo`). On hover it takes the active character's `--title-accent` (set by `setActive()`; `gumi-title.js` only watches `#gumizoo`'s style attribute), lifted in OKLCH: same hue, lightness at least .68, chroma ×1.35 within sRGB. So every hover reads clearly brighter and more saturated than the gray and stays in its hue family (measured face chroma .12–.16 against the gray's .01, hue within about 3° of the accent). The same formula is in CSS relative-colour syntax for the fallback. The side wall and outline use darker shades of the same tint, so edges measure about 4.3:1 against the cream at rest and 6–8:1 on hover. The pearl face itself is about 1.5:1, a deliberate exception to the 4.5:1 rule, approved; the edges carry legibility and the accessible text is unaffected. The recolour is a .3s tween, instant under reduced motion.
       - **Cost:** built once when the section is within 600px of the viewport, then redrawn only on a colour change or a resize (no idle loop), at a device-pixel ratio capped at 2.
       - **Fallback:** no WebGL, a lost context, a failed shader, Bagel Fat One not loading, or forced colours all leave the CSS version: the same text and colours, a darker `-webkit-text-stroke` and a stepped darker `text-shadow` extrusion, with the same transition. Forced colours gets plain system text.
