...
3. **Selected work:** personal projects.
   ...
   - Gumizoo: a centered composition on the site's plain cream background (no section background of its own) — "Gumizoo" heading (thick, glossy gummy-candy lettering, see "The gummy title" below; about the height of the old Bebas line box, so it never outweighs the heads) and the tagline "Four forms of stupidity" beneath it, both centered; below that the four heads (Ashik Rancher, Gia Tangerine, Nanju Jam, Rana Ghee) at their real render aspect (`--gumi-ar`, 1230:963 — uncropped, no letterboxing), 2×2 on phones and one row of four on desktop (900px+), also centered; a shared materials note ("PLA+ filament, wood filler, primer, acrylic paint.") below that, centered too. The event poster asset (`media/stills/gumizoo.webp`) is no longer part of this section's visible layout — `main.js` still unconditionally fetches and appends it to `.gumizoo__poster` on init, so that element stays in the DOM, just `display: none`. Desktop: sized to fit one viewport with no scroll, the same `min()`-of-a-width-and-a-height-budget approach as before, just without a poster term. Four-across is more width-hungry than the old 2×2 was, so — with no poster competing for width — this row ends up width-bound rather than height-bound, hitting the standard `--max` (1400px) cap on wide/short screens with a lot of spare height left over (which the section's own vertical centering turns into calm whitespace, not a cramped row): tiles come out to 338×265px at 1920×1080 (row at 72.9% of viewport, the full 1400px cap) and 292×229px at 1366×768 (row at 89.0% of viewport, gutter-limited there, short of the cap). Gaps: 16px on desktop (12px on phones). Frames: 2px ink border (`rgba(17,17,17,.78)`) and a soft drop shadow. Tile texture: at rest a light neutral grainy gradient (off-white, grain at 42% opacity, `soft-light` blend — not `multiply`, which only ever darkens and read too heavy/saturated at rest); on hover/focus that tile's accent gradient fades in under the same grain (opacity up to .5), so it's one texture going from quiet to bold. The section background never floods; only the hovered tile does. Phones: heading, tagline, grid, materials note stacked, still centered.
     - **The gummy title is a Blender render too, tinted live** (no WebGL, no new library, no new web font). GUMIZOO is set in Bagel Fat One (OFL, used only offline for the render; the live heading text and its fallback stay Bebas Neue), letter spacing 1.22, extruded 0.12 with a 0.05 round bevel, voxel-remeshed (0.004) and smoothed (12 iterations) into separate pillowy candies, seen orthographically from 13° below so the thickness shows under each letter. The material is a neutral white gummy (subsurface .7, transmission .3, roughness .16, coat .7 at .04), lit by a small upper-left key, a grazing top light, a back light and a dim grey world. It is rendered three times at 1600px (192 Cycles samples, denoised), once per pass, then levelled and exported as grayscale WebP with alpha: `media/stills/gumizoo-title-shade.webp` (the lit form, whose alpha is the letter mask), `-glow.webp` (light through the candy, as alpha) and `-gloss.webp` (white highlights, as alpha, kept off the outer silhouette). CSS (`.gumi-title`) fills the letters with `var(--title-accent, #111111)`, multiplies the shade over it, lets a lighter accent through the glow (`color-mix` with 40% white) and lays the gloss on top. So the existing hover recolour (`setActive()` sets `--title-accent`) drives it unchanged, including the .3s transition (instant under reduced motion); at rest it's a glossy black candy. `main.js` adds `.is-candy` to `#gumizoo` only once all three passes have decoded. Until then, if one fails, and under forced colours, the plain Bebas heading (stepped ink `text-shadow`, same recolour) shows instead. The accessible name is always the real text "Gumizoo". The Blender/Python scripts lived in the building session's scratch folder (run via `pip install bpy`); rebuild them to these numbers if the word or font ever changes.
