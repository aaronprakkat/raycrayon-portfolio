...
3. **Selected work:** personal projects.
   ...
   - Gumizoo, per Ryan's layout sketch: "Gumizoo" heading top-left with the tagline "Four forms of stupidity" beneath it, and a short intro line beside that heading stack (Inter body copy, 52ch max — same composition as `.project-head`/Pangeo's heading-beside-description, not a plain stack; PROPOSED COPY pending Ryan's approval, flagged in an HTML comment above it in index.html, same convention as `bioDraft` in js/projects.js); below that a 2×2 grid of square hairline frames, one per head (Ashik Rancher, Gia Tangerine, Nanju Jam, Rana Ghee); beside the grid the event poster (`media/stills/gumizoo.webp`, 9:16) at the grid's full height, static; below the grid/poster block, one shared materials note ("PLA+ filament, wood filler, primer, acrylic paint."). All inside the standard `.wrap`, using the same `--max`/`--gutter` container as the rest of the site, left-aligned at the standard gutter like every other section's heading (not centred — centring only reads right once a block is close to the container's full width, and this one usually isn't). Desktop (900px+): the head row (heading, tagline and intro line) spans the full standard content width, same as Pangeo's; the grid+poster block below it is what actually fits one viewport with no scroll: work backward from the height left over once the section's own padding and the real rendered height of the heading+tagline and the materials note are subtracted (each accounted for with the exact `clamp()` expression that field's type uses, not a guess — the intro line doesn't need its own term, since even at 2-3 lines of body copy it stays shorter than the heading stack beside it), and only then does the standard width cap (`--max`) get a say, as a ceiling so the block never runs wider than the site's usual container on a very tall, very wide screen. The tiles and poster come down in size on shorter screens rather than the section ever scrolling — legibility still matters, but fitting in one screen takes priority. Gaps (heading to grid, between tiles, and grid to poster/materials) are the brand grid gutter: 24px desktop, 12px phones. Frames (tiles and poster alike): 2px ink border (`rgba(17,17,17,.78)`) and a soft drop shadow. Tile texture: at rest a light neutral grainy gradient (off-white, grain at 42% opacity, `soft-light` blend — not `multiply`, which only ever darkens and read too heavy/saturated at rest); on hover/focus that tile's accent gradient fades in under the same grain (opacity up to .5), so it's one texture going from quiet to bold. The section background never floods; only the hovered tile does. Phones: heading, tagline, intro line, grid, poster, materials note stacked.
     ...
     - Tiles show the still. Hover/focus (mouse, keyboard): the head lifts out of its frame (rises, scales 1.2, tilts -4°, drop shadow), its nameplate pops up centred over the head: a chunky toy-shop sticker (accent fill, 3px ink outline, hard offset shadow, Bebas, surname smaller), tilted -5°/+4° alternately, bouncing in with a wobble, that tile floods with its accent (no fragments), and the GUMIZOO heading turns that character's accent (black at rest, and again when nothing is active). The hover also starts fetching that head's turntable. Touch: no hover; nameplates always shown (smaller, low on the tile); tap opens the panel.
     ...
     - The turntable sprite is decoded ahead of time, not at the moment the panel opens: the first time a ~40-megapixel sheet is ever actually painted (as opposed to just fetched) costs a real, measured ~300-400ms — a visible stall right in the transition. `warmSprite()` (main.js) pays that cost as soon as the sprite finishes loading (on hover or keyboard focus, same trigger as `loadSprite`), using a real but invisible (`opacity: .01`, not `0` — a fully transparent element can skip the actual paint/decode work as a browser optimisation) composited element sized close to the panel's actual on-screen size, so the decode has already happened by the time a real hover/focus-then-click reaches the panel.
