...
3. **Selected work:** personal projects.
   ...
   - Gumizoo, per Ryan's layout sketch: "Gumizoo" heading top-left; below it a 2×2 grid of square hairline frames, one per head (Ashik Rancher, Gia Tangerine, Nanju Jam, Rana Ghee); beside the grid the event poster (`media/stills/gumizoo.webp`, 9:16) at the grid's full height, static. All inside the standard `.wrap`. Desktop (900px+): framed as one screen: the section is a viewport tall under the nav, the block is centred in it with equal space left and right and 40px above and below, and it's sized from the viewport height as well as the width so the heading, all four tiles and the poster are always visible together. Gaps (heading to grid, between tiles, and grid to poster) are the brand grid gutter: 24px desktop, 12px phones. Frames (tiles and poster alike): 2px ink border (`rgba(17,17,17,.78)`) and a soft drop shadow. Tile texture: at rest a light neutral grainy gradient (off-white, grain at 42% multiply, visible on its own); on hover/focus that tile's accent gradient fades in under the same grain, so it's one texture going from quiet to bold. The section background never floods; only the hovered tile does. Phones: heading, grid, poster stacked.
     ...
     - Assets per head: `media/stills/gumizoo-<slug>-3d.webp` (still, frame 0), `gumizoo-<slug>-turn.webp` (36 frames, 10° apart, as a 6×6 sprite; an exception to the 2000px stills rule) and `-turn-480.webp` (the same at 60% for phones). All four are normalised to the same front width and centre, and every frame is cropped to one shared tight box (symmetric about the turning axis), whose aspect is `--gumi-ar` on `:root`. So one CSS rule sizes every head identically: 95% of the tile's width, and the full width of the panel's model column.
     - Tiles show the still. Hover/focus (mouse, keyboard): the head lifts out of its frame (rises, scales 1.2, tilts -4°, drop shadow), its nameplate pops up centred over the head: a chunky toy-shop sticker (accent fill, 3px ink outline, hard offset shadow, Bebas, surname smaller), tilted -5°/+4° alternately, bouncing in with a wobble, that tile floods with its accent (no fragments), and the GUMIZOO heading turns that character's accent (black at rest, and again when nothing is hovered). The hover also starts fetching that head's turntable. Touch: no hover; nameplates always shown (smaller, low on the tile); tap opens the panel.
     - Click: one full 360° turn of the head is the transition into the detail panel (it runs inside the View Transitions morph from tile to panel). In the panel the head is the dominant element (a 1.8fr : 1fr column split, 1280px max sheet); drag across the head (its width = one full turn) or use the arrow keys (it's a `role="slider"` with degrees as the value) to turn it. Closing turns it back to face front, then morphs back to the tile.
